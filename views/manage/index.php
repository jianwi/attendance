...
<div class="panel panel-primary">
    ...
    <div class="panel-body">
        <a href="?r=manage/group-manage" class="btn btn-primary">组员管理</a>
        <a href="?r=manage/branch" class="btn btn-primary">添加分支</a>
...
<div class="panel panel-primary" style="">
        ...
        <div class="panel panel-body">
            <table class="table table-hover">
                ...
                <tr>
                    <td>名称</td>
                    <td id="group_name" onclick="edit(this)"><?=$info['name']?></td>
                </tr>
                <tr>
                    <td>所在学校</td>
                    <td id="group_school" onclick="edit(this)"><?=$info['school']?></td>
                </tr>
                <tr>
                    <td>考勤规则</td>
                    <td id="group_detail" onclick="edit(this)"><?=$info['detail']?></td>
                </tr>
                <tr>
                    <td>考勤地点</td>
                    <td data-toggle="modal" data-target="#map_modal" id="group_location" onclick="signLoc()">设置/修改</td>
                </tr>
<!--                <tr>-->
<!--                    <td>考勤组状态</td>-->
<!--                    <td>--><?//=$info['state']?><!--</td>-->
<!--                </tr>-->
                <tr>
                    <td colspan="2"><button class="btn btn-default" data-toggle="modal" data-target="#modify_group_info" onclick="modifyGroup()">修改信息</button></td>
                </tr>
                <textarea id="edit_box" onblur="editHide()" rows="2" style="position: absolute;display: none"></textarea>
            </table>
        </div>
</div>
...
<script>
    ...
    * */
    function edit(element) {
        if (edit_box.value!="") {
            document.getElementById(edit_box.name).innerHTML = edit_box.value;
        }
        var left=getElementLeft(element);
        var top=getElementTop(element);
        edit_box.style.top=top+"px";
        edit_box.style.left=left+"px";
        edit_box.value=element.textContent;
        window.ele=element;
        edit_box.style.display="block"
        edit_box.name=element.id
    }
    function editHide() {
        if (edit_box.value !=="") {
            // alert(22)
    ...
    function modifyGroup() {
        editHide()
        $.post("?r=manage/modify-group-info",{
            id:<?=$info['id']?>,
            name:group_name.textContent,
            school:group_school.textContent,
            detail:group_detail.textContent,
            location:JSON.stringify(LOC.sign),
        },function (data) {
            if (data>0){
                alert("修改成功");
            }else {
...
<script>
   ...
   function loadLeave(ele,state){
       ...
       switch (state) {
           ...
           var url=`?r=manage/leave-list&limit=5&offset=${offset}&state=${state}`;
            offset==null;
    ...
    function checkLeave(ele,state){
       ...
       var url=`?r=manage/check-leave&id=${id}&state=${state}`;
    ...
    function loadSign(ele,state) {
        ele.disabled=true;
        ele.textContent="加载中，君莫急呢";
        $.getJSON('?r=manage/sign-list&limit=20&offset='+loadSign.offset,function (data){
            if(data.length==0){
...
<script type="text/javascript"
        src="https://webapi.amap.com/maps?v=1.4.13&key=36460e9c265d71750a1988db3f1681ab&plugin=AMap.CircleEditor"></script>
<script>
    var LOC={}
    initLoc();
    ...
    function drawCircle(map) {
        ...
        if ("undefined"!==typeof LOC.sign.radius) {
            // map.setCenter(LOC.sign.position.lng,LOC.position.lat)
